...
> 当前版本：**0.5.2**。macOS 已提供可直接运行的 Apple Silicon 安装包；Windows 版本提供完整源码和发布脚本，需要在 Windows 上构建。
...
项目图标与气泡使用同一套 Wonder 视觉识别：白底圆角、蓝紫液态玻璃渐变，以及 W 与星星元素。

![Wonder 图标](assets/wonder-icon-master.png)
...
[Wonder-v0.5.2-macOS-arm64.zip](releases/Wonder-v0.5.2-macOS-arm64.zip)

解压后将 `Wonder.app` 拖入“应用程序”，双击启动即可。首次使用划词功能时，请在：

**系统设置 → 隐私与安全性 → 辅助功能**

中允许 Wonder 控制电脑。Wonder 不会主动索要系统密码，也不会把辅助功能权限当作翻译接口密钥。

如果 macOS 提示应用来自未验证开发者，可在 Finder 中右键点击应用并选择“打开”。这是当前开发签名包的正常提示；正式公开分发时建议使用 Apple Developer ID 签名和公证。

### Windows

Windows 版本目前以源码形式交付，需要在 Windows 10 1903+ 或 Windows 11 上使用 .NET 8 SDK 构建：
...
- Windows 程序必须在 Windows 环境中构建，因为 WPF 和 Windows UI Automation 依赖 Windows Desktop SDK。
